...
Copyright 1996-2012 Adobe, Inc. All Rights Reserved. More info available at http://www.omniture.com */
...
s.linkInternalFilters="javascript:,akagourmet,wine.com"
...
function s_doPlugins() {
    ...
    //campaign tracking s_campaign
    var source = s.getQueryParam('cid');
    if (source) {
        if (source.length <= 0) {
            source = s.getQueryParam('s');
        }
    } else {
        source = s.getQueryParam('s');
    }
    ...
    if (source) {
        if (source.indexOf("_") > 0) {
            ...
            if (sourceParts.length >= 1) {
                s.prop25 = sourceParts[0];
                s.eVar36 = sourceParts[0];

                if (sourceParts.length >= 2) {
                    s.prop26 = sourceParts[1];
                    s.eVar37 = sourceParts[1];
                }
            }
        }
    }
	
	//keyword tracking evar2
    if ((s.eVar2 == null) || (s.eVar2.length == 0)) {
        s.eVar2 = s.getQueryParam('kid');
    }
	
	//master_category evar4
	//s.eVar4 = s.getQueryParam('mid');
	
	//sub_category evar5
	//s.eVar5 = s.getQueryParam('sid');
	
	//internal campaign evar7
	if ((s.eVar7 == null) || (s.eVar7.length == 0)) {
	    var iid = s.getQueryParam('iid');
	    if (iid == "") {
	        //Sociable Labs code. Remove this if Sociable Labs is removed.
	        iid = s.getQueryParam('SSFEAT');
	    }
	    s.eVar7 = iid;
	    s.eVar17 = iid;
	    s.eVar18 = iid;
	...
	//ongoing marketing initiatives evar8
	if ((s.eVar8 == null) || (s.eVar8.length == 0)) {
	    s.eVar8 = s.getQueryParam('hid');
	}
...
changes to how your visitor data is collected.  Changes should only be
made when instructed to do so by your account manager.*/
s.trackingServer="winecom.112.2o7.net"
